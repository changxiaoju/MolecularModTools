...
class fit:
    ...
    def fit(self,time,ave,stddev,use_double_exp,popt2,std_perc,endt):
        ...
        if use_double_exp:
            ...
            popt2,pcov2 = optimize.curve_fit(
                self.doubleexp, time[start:cut], ave[start:cut],maxfev=1000000,p0=popt2, sigma=stddev[start:cut],bounds=(0,[np.inf,1,np.inf,np.inf]))
            fit = self.doubleexp(time, *popt2)
            Value = popt2[0]*popt2[1]*popt2[2]+popt2[0]*(1-popt2[1])*popt2[3]
        else:
            # Use single exponential fit
            popt2, pcov2 = optimize.curve_fit(
                self.singleexp, time[start:cut], ave[start:cut], maxfev=1000000, p0=popt2, sigma=stddev[start:cut],
